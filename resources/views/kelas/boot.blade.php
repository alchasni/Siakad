...
<html>
      ...
      <table id="grid-basic" class="w3-table-all w3-card-4">
         <thead>
            <tr class="w3-cyan">
               ...
               <th data-column-id="namamatkul" >Nama Matkul</th>               
   ...
   <div id="edit" class="w3-modal">
         <div class="w3-modal-content w3-card-8 w3-animate-zoom" style="max-width:600px">
            ...
            <form class="w3-container" action="/savkelas" method = "POST">
              {{ csrf_field() }}
              <input type="hidden" id="edit_id" name="edit_id">
               <div class="w3-section">
                  <div class="w3-row">
                     <div class="w3-col s4"><label><b>Kode Kelas</b></label></div>
                     <div class="w3-col s8"><input class="w3-input w3-border w3-margin-bottom" id="kodekelas" type="text" placeholder="Enter Kode Kelas" name="kodekelas" required>
                     </div>
                  </div>
      ...
      <div id="delete" class="w3-modal">
         <div class="w3-modal-content w3-card-8 w3-animate-zoom" style="max-width:600px">
            ...
            <form class="w3-container" action="/delkelas" method="POST">
              {{ csrf_field() }}
         ...
         $( document ).ready(function(){
           $("#grid-basic").bootgrid({
             formatters: {
               ...
               {
                 return "<button onclick=\"document.getElementById('edit').style.display='block'\" data-id=\"" + row.id + "\" data-kodekelas=\"" + row.kodekelas + "\" data-namamatkul=\"" + row.namamatkul + "\" class=\"w3-btn w3-blue w3-small edit\"><span class=\"fa fa-pencil\"></span></button> " +
                 "<button onclick=\"document.getElementById('delete').style.display='block'\" data-kodekelas=\"" + row.kodekelas + "\" data-namamatkul=\"" + row.namamatkul  + "\" class=\"w3-btn w3-blue w3-small delete\"><span class=\"fa fa-remove\"></span></button>";
               }
             }}).on("loaded.rs.jquery.bootgrid", function (){
               /* Executes after data is loaded and rendered */
               $(this).find(".edit").click(function (e) {
                 $('#edit_id').val($(this).data("kodekelas"));
                 $('#kodekelas').val($(this).data("kodekelas"));
                 $('#namamatkul').val($(this).data("namamatkul"));
                 
               });
               $(this).find(".delete").click(function (e) {
                 $('#del_id').val($(this).data("kodekelas"));
                 $('#delete_name').html($(this).data("kodekelas") +" "+ $(this).data("namamatkul"));
               });
             });
           });
      </script>
